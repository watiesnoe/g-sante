            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body text-center py-4">
                        <div class="avatar-lg bg-pink-light rounded-circle mx-auto mb-3 d-flex align-items-center justify-content-center"
                            style="width: 80px; height: 80px; background-color: #fce4ec; color: #e91e63;">
                    ...
                    <div class="list-group list-group-flush border-top">
                        <div class="list-group-item d-flex justify-content-between">
                            ...
                            <span
                                class="fw-bold">{{ \Carbon\Carbon::parse($grossesse->patient->date_naissance)->age ?? '?' }}
                                ans</span>
                        </div>
                        <div class="list-group-item d-flex justify-content-between">
                            <span class="text-muted">DDR</span>
                            <span
                                class="fw-bold text-primary">{{ \Carbon\Carbon::parse($grossesse->ddr)->format('d-m-Y') }}</span>
                        </div>
                        <div class="list-group-item d-flex justify-content-between">
                            <span class="text-muted">DPA Estimée</span>
                            <span
                                class="fw-bold text-danger">{{ \Carbon\Carbon::parse($grossesse->dpa)->format('d-m-Y') }}</span>
                        </div>
                        <div class="list-group-item d-flex justify-content-between">
                            <span class="text-muted">Terme actuel</span>
                            <span
                                class="badge bg-info text-white">{{ round(\Carbon\Carbon::parse($grossesse->ddr)->diffInWeeks(now())) }}
                                SA</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mt-4">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            <a href="{{ route('consultations.create', ['patient_id' => $grossesse->patient_id, 'grossesse_id' => $grossesse->id]) }}"
                            ...
                            <a href="{{ route('ordonnances.maternite', ['patient' => $grossesse->patient->uuid]) }}"
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @forelse($grossesse->cpns->sortBy('numero_cpn') as $cpn)
                                        <tr>
                                            <td><span class="badge bg-secondary rounded-pill">CPN
                                                    {{ $cpn->numero_cpn }}</span></td>
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="number" name="numero_cpn" class="form-control"
                                    value="{{ $grossesse->cpns->count() + 1 }}" required>
                            </div>
                            <div class="col-md-5">
                                <label class="form-label fw-bold small">Date de visite</label>
                                <input type="date" name="date_cpn" class="form-control" value="{{ date('Y-m-d') }}"
                                    required>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="bcf" class="form-control"
                                    placeholder="Bruit Coeur Foetal">
                            </div>

                            <div class="col-md-12">
                                <label class="form-label fw-bold small">Traitement reçu / Supplémentation</label>
                                <input type="text" name="traitement_recu" class="form-control"
                                    placeholder="ex: Fer, Acide Folique, Moustiquaire...">
                            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="issue" class="form-select" required>
                                ...
                                <option value="Interruption Médicale de Grossesse (IMG)">Interruption Médicale de Grossesse
                                    (IMG)</option>
                            </select>
                        </div>
                        <div class="mb-3">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-info text-white">
                    <h5 class="modal-title fw-bold" id="modalRendezvousLabel">
                        <i class="fa fa-calendar-plus me-2"></i>Fixer un Rendez-vous
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    ...
                    {{-- Infos patient --}}
                    <div class="alert alert-info border-0 d-flex align-items-center mb-3 py-2">
                        <i class="fa fa-user-circle fa-2x me-3 text-info"></i>
                        <div>
                            <div class="fw-bold">{{ $grossesse->patient->nom }} {{ $grossesse->patient->prenom }}</div>
                            <small class="text-muted">Patient lié automatiquement</small>
                        </div>
                    </div>

                    <form id="formRdvMaternite">
                        @csrf
                        {{-- Patient caché (UUID) --}}
                        <input type="hidden" name="patient_uuid" value="{{ $grossesse->patient->uuid }}">

                        {{-- Médecin : utilisateur connecté (UUID) --}}
                        <input type="hidden" name="medecin_uuid" value="{{ auth()->user()->uuid }}">
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="motif"
                                      ...
                                      class="form-control"
                                      rows="2"
                                      placeholder="Ex: CPN, contrôle post-natal, suivi grossesse..."></textarea>
                        </div>
                    </form>
                ...
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <button type="button" class="btn btn-info text-white px-4" id="btnSaveRdv">
...
$(document).ready(function () {
    $('#btnSaveRdv').on('click', function () {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: $('#formRdvMaternite').serialize(),
            success: function (res) {
                if (res.success) {
                    $('#modalRendezvous').modal('hide');
                    Swal.fire({
                        icon: 'success',
                        title: 'Succès !',
            ...
            complete: function () {
                btn.prop('disabled', false).html('<i class="fa fa-save me-1"></i> Enregistrer');
            }
        });
    });
});
