                                ...
                                <select id="patientDropdown" class="form-select form-select-sm border-0" required>
                                    ...
                                    @foreach ($tickets as $ticket)
                                        <option value="{{ $ticket->patient->id }}" data-ticket="{{ $ticket->id }}"
                                            @if (($consultation && $consultation->patient_id == $ticket->patient->id) || (isset($selectedTicketId) && $selectedTicketId == $ticket->id)) selected @endif>
                                            {{ $ticket->patient->nom }} {{ $ticket->patient->prenom }} - Ticket N° {{ $ticket->id }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="card-body p-4 bg-light">
                        ...
                        <div class="card shadow-sm mb-4 border-0">
                            ...
                            <div class="card-body pt-0">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="form-label fw-semibold small text-secondary">Motif de consultation</label>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="antecedents" class="form-control" rows="2" placeholder="Antécédents médicaux, allergies, traitements en cours...">{{ $consultation->antecedents ?? '' }}</textarea>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label fw-semibold small text-secondary">Poids (kg)</label>
                                        <input type="number" name="poids" class="form-control" min="1" step="0.1" value="{{ $consultation->poids ?? '' }}" placeholder="kg">
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label fw-semibold small text-secondary">Taille (cm)</label>
                                        <input type="number" name="taille" class="form-control" min="30" step="0.1" value="{{ $consultation->taille ?? '' }}" placeholder="cm">
                                    </div>
                                    <div class="col-md-3">
                        ...
                        <div class="card shadow-sm mb-4 border-0">
                            ...
                            <div class="card-body">
                                ...
                                <div id="diagnosticAssistantContainer" class="bg-white rounded shadow-sm p-3 mb-3" style="min-height:120px;">
                                    <div id="diagnosticAssistantEmpty" class="text-center py-4 text-muted small">
                                        ...
                                        Cochez les symptômes dans le panneau latéral <strong>→</strong> pour lancer l'analyse.
                                    </div>
                                ...
                                <div class="row g-3 mt-4 pt-4 border-top">
                                    <div class="col-md-5">
                                        ...
                                        <select name="maladie_id" id="maladieSelect" class="form-select js-select2" required>
                                        ...
                                        <div id="protocoleContainer" class="mt-3" style="display:none;">
                                            <div class="card border-0 shadow-lg border-start border-5 border-warning overflow-hidden bg-white">
                                                ...
                                                <div class="card-body p-3">
                                                    <div class="small fw-bold text-success text-uppercase mb-1">Traitement recommandé :</div>
                                                    ...
                                                    <div class="d-none" id="multiProtocolSelector">
                                                        <select id="selectProtocoleAlt" class="form-select form-select-sm mt-2"></select>
                                                    </div>
                                                    <button type="button" id="btnApplyProtocole" class="btn btn-success btn-sm w-100 shadow-sm mt-2">
                                                        <i class="fas fa-check-circle me-1"></i>Appliquer à l'Ordonnance
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-7">
                                        <label class="form-label fw-bold small text-secondary">RÉSUMÉ CLINIQUE / OBSERVATIONS</label>
                                        <textarea class="form-control" name="diagnostic" placeholder="Saisir la synthèse de la consultation..." rows="5" required>{{ $consultation->diagnostic ?? '' }}</textarea>
                                    </div>
                                </div>
                            ...
                            <div class="card-body pt-0">
                                <div class="mb-4">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <button type="button" id="btnAjouterMedicament" class="btn btn-primary btn-sm">
                                                    ...
                                                    @foreach ($consultation->ordonnances as $ordonnance)
                                                        @foreach ($ordonnance->medicaments as $medicament)
                                                            <tr>
                                                                ...
                                                                <td><input type="text" name="posologies[]" class="form-control form-control-sm" value="{{ $medicament->pivot->posologie }}" required></td>
                                                                <td><input type="number" name="duree_jours[]" class="form-control form-control-sm" value="{{ $medicament->pivot->duree_jours }}"></td>
                                                                <td><input type="number" name="quantites[]" class="form-control form-control-sm input-qty" value="{{ $medicament->pivot->quantite }}"></td>
                                    ...
                                    <div class="col-md-6">
                                        <div class="border rounded p-3 bg-white">
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <button type="button" id="btnAjouterAnalyse" class="btn btn-warning btn-sm">
                                            ...
                                            <ul class="list-group list-group-flush" id="analyseList">
                                                @forelse($consultation->examens ?? [] as $ex)
                                                    <li class="list-group-item px-0 d-flex gap-2 align-items-center">
                                                        ...
                                                        <input type="text" name="examens[]" class="form-control flex-grow-1" value="{{ $ex->examen }}" required>
                                    ...
                                    <div class="col-md-6">
                                        <div class="border rounded p-3 bg-white">
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <button type="button" id="btnAjouterRdv" class="btn btn-secondary btn-sm">
                                                    <i class="fas fa-plus me-1"></i> Ajouter
                                                </button>
                                            </div>
                                            <ul class="list-group list-group-flush" id="rdvList">
                                                @forelse($consultation->rendezVous ?? [] as $r)
                                                    <li class="list-group-item px-0">
                                                        <div class="row g-2">
                                                            <div class="col-5"><input type="text" name="rdv_motifs[]" class="form-control" value="{{ $r->motif }}" required></div>
                                                            <div class="col-3"><input type="date" name="rdv_dates[]" class="form-control" value="{{ \Carbon\Carbon::parse($r->date_heure)->format('Y-m-d') }}" required></div>
                                                            <div class="col-3"><input type="time" name="rdv_heures[]" class="form-control" value="{{ \Carbon\Carbon::parse($r->date_heure)->format('H:i') }}" required></div>
                                                            <div class="col-1"><button type="button" class="btn btn-link text-danger btn-sm btnSupprimerRdv p-0"><i class="fas fa-trash-alt"></i></button></div>
                                                        </div>
                                                    </li>
                                                @empty
                        ...
                        <div class="card shadow-sm mb-4 border-0">
                            ...
                            <div class="card-body pt-0">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <textarea class="form-control" name="certificat" placeholder="Certificat médical (optionnel)..." rows="2">{{ $consultation->certificat->contenu ?? '' }}</textarea>
                                    </div>
                                    <div class="col-12">
                                    ...
                                    <div id="hospitalisationFields" class="col-12" style="{{ $consultation && $consultation->hospitalisation ? '' : 'display:none;' }}">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <select id="salleSelect" name="salle_id" class="form-select js-select2">
                                                    <option value="">-- Sélectionner --</option>
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="text" name="observations" class="form-control" placeholder="Observations" value="{{ $consultation->hospitalisation->observations ?? '' }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="card shadow-sm border-0 sticky-top" style="top:70px; z-index:10;">
                    ...
                    <div class="card-header bg-dark text-white p-0 border-0">
                        <ul class="nav nav-tabs nav-fill border-0" role="tablist">
                            <li class="nav-item">
                                <button class="nav-link active text-white border-0 rounded-0 py-2 small fw-bold" style="background:transparent;" data-bs-toggle="tab" data-bs-target="#tabSymptomes" type="button">
                                    <i class="fas fa-stethoscope me-1"></i> Symptômes
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link text-white border-0 rounded-0 py-2 small fw-bold" style="background:transparent;" data-bs-toggle="tab" data-bs-target="#tabAtlas" type="button">
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade show active" id="tabSymptomes">
                            <div class="p-2 border-bottom bg-light">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <span class="small fw-bold text-success"><i class="fas fa-check-circle me-1"></i>Interrogatoire</span>
                                    <button type="button" id="btnResetQuestions" class="btn btn-outline-secondary py-0 px-2" style="font-size:0.65rem;">Réinitialiser</button>
                                </div>
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-white border-end-0 py-0"><i class="fas fa-search text-muted" style="font-size:0.7rem;"></i></span>
                                    <input type="text" id="symptomSearch" class="form-control border-start-0 py-1" placeholder="Rechercher..." style="font-size:0.8rem;">
                                </div>
                            </div>
                            <div class="overflow-auto" id="symptomListSidebar" style="max-height: calc(100vh - 220px); scrollbar-width:thin;">
                                @foreach ($symptomes as $symptome)
                                    <div class="symptom-item border-bottom">
                                        <label class="d-flex justify-content-between align-items-center px-3 py-2 m-0 symptom-sidebar-label" for="s_check_{{ $symptome->id }}" style="cursor:pointer;">
                                            <span class="small text-dark symptom-label">{{ $symptome->nom }}</span>
                                            <div class="form-check form-switch m-0">
                                                <input class="form-check-input symptom-checkbox shadow-sm" type="checkbox"
                                                    id="s_check_{{ $symptome->id }}" value="yes"
                        ...
                        <div class="tab-pane fade" id="tabAtlas">
                            <div class="p-2 border-bottom bg-light">
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-white border-end-0 py-0"><i class="fas fa-search text-muted" style="font-size:0.7rem;"></i></span>
                                    <input type="text" id="atlasSearch" class="form-control border-start-0 py-1" placeholder="Chercher une maladie..." style="font-size:0.8rem;">
                                </div>
                            </div>
                            <div class="list-group list-group-flush overflow-auto" id="atlasList" style="max-height: calc(100vh - 220px);">
                                @foreach($maladies as $m)
                                    @if($m->protocole)
                                        <div class="list-group-item p-0 border-0 mb-1">
                                            <div class="d-flex align-items-center">
                                                <div class="ps-2">
                                                    <input type="checkbox" class="form-check-input compare-check" data-id="{{ $m->id }}" data-nom="{{ $m->nom }}" style="width:0.9em;height:0.9em;">
                                                </div>
                                                <a href="javascript:void(0)" class="list-group-item-action px-2 py-2 flex-grow-1 select-atlas-maladie" data-id="{{ $m->id }}">
                                                    <span class="fw-bold text-dark" style="font-size:0.8rem;">{{ $m->nom }}</span>
                                                    <div class="text-muted text-truncate" style="font-size:0.65rem;">
                                                        <i class="fas fa-pills me-1 text-success"></i> {{ $m->protocole->traitement_principal }}
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                    ...
                    <div id="compareBar" class="card-footer bg-primary text-white py-2 d-none">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" id="btnLaunchCompare" class="btn btn-sm btn-light py-0">Comparer <i class="fas fa-columns ms-1"></i></button>
                        </div>
                    </div>
                    <div class="card-footer bg-light py-1 text-center">
                        <small class="text-muted" style="font-size:0.6rem;">Symptômes → IA • Atlas → Ordonnance auto</small>
                    </div>
                </div>
            </div>{{-- fin col-lg-3 --}}
                    ...
                    <table class="table table-bordered mb-0" id="compareTable" style="table-layout:fixed;">
                        <thead class="bg-light align-middle text-center">
                            <tr>
                                ...
                                <th id="compTitle1" class="text-primary fw-bold" style="width:42.5%">Pathologie A</th>
                                <th id="compTitle2" class="text-success fw-bold" style="width:42.5%">Pathologie B</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-center p-3">
                                    <button type="button" class="btn btn-primary btn-sm rounded-pill" id="btnApplyComp1">Appliquer cette Ordonnance</button>
                                </td>
                                <td class="text-center p-3">
                                    <button type="button" class="btn btn-success btn-sm rounded-pill" id="btnApplyComp2">Appliquer cette Ordonnance</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
═══════════════════════════════════════════════════════════════ --}}
<style>
    .list-group-item-action:hover { background-color: #f8fafc; border-left: 3px solid #0061f2; }
    .bg-soft-success { background-color: rgba(25, 135, 84, 0.1) !important; }
    #compareTable td { vertical-align: top; line-height: 1.6; }
    /* Sidebar symptom items */
    .symptom-sidebar-label:hover { background-color: #f0f4ff; }
    .symptom-item.bg-soft-success .symptom-label { font-weight: 600; color: #198754 !important; }
    /* Active tab */
    .nav-tabs .nav-link.active { background: rgba(255,255,255,0.15) !important; border-bottom: 3px solid #fff !important; }
    .nav-tabs .nav-link:hover:not(.active) { background: rgba(255,255,255,0.08) !important; }
    /* Diagnostic cards */
    .select-suggested-maladie:hover { transform: translateY(-3px); background-color: #f8f9fa; }
</style>
@endsection
...
$(document).ready(function() {
    ...
    // 3. HOSPITALISATION
    // ═══════════════════════════════════════════════════════════
    $('#hospitalisationCheck').change(function() { $('#hospitalisationFields').toggle(this.checked); });

    function chargerLitsPourSalle(salleId, litId = null) {
        let litSelect = $('#litSelect');
        let currentLitId = litId || $('#lit_id').val();
        if (!salleId) { litSelect.html('<option value="">-- Sélectionner un lit --</option>'); return; }
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    $.each(response, function(i, lit) {
                        ...
                        litSelect.append(`<option value="${lit.id}" ${sel}>Lit #${lit.numero}</option>`);
                    });
                } else { litSelect.append('<option value="">Aucun lit disponible</option>'); }
            },
            error: function() { litSelect.html('<option value="">Erreur de chargement</option>').prop('disabled', false); }
        });
    }
    $('#salleSelect').change(function() { chargerLitsPourSalle($(this).val()); });
    ...
    $('#symptomes').change(function() {
        ...
        if (selectedSymptomes.length === 0) { emptyState.show(); content.hide(); return; }
        emptyState.hide(); content.show();
        ...
        for (let maladieId in maladieSymptomesDetails) {
            ...
            if (inter.length > 0) {
                scores.push({ id: maladieId, nom: maladie.nom, score: Math.round((inter.length / req.length) * 100), matchedCount: inter.length, totalCount: req.length });
            }
        }
        ...
        if (scores.length > 0) {
            ...
            if (scores[0].score >= 80 && $('#maladieSelect').val() != scores[0].id) {
                Toast.fire({ icon: 'info', title: `Forte suspicion de ${scores[0].nom}` });
            }
        } else {
    ...
    $('#maladieSelect').change(function() {
        ...
        if (!maladieId) { $('#protocoleContainer').hide(); return; }
            ...
            success: function(response) {
                if (response.success && response.protocoles && response.protocoles.length > 0) {
                    ...
                    if (protocols.length > 1) {
                        let opts = '';
                        protocols.forEach((item, idx) => { opts += `<option value="${idx}">${item.titre}</option>`; });
                        $('#selectProtocoleAlt').html(opts);
                        $('#multiProtocolSelector').show();
                        $('#selectProtocoleAlt').off('change').on('change', function() { updateProtocoleUI(protocols[$(this).val()]); });
                    } else { $('#multiProtocolSelector').hide(); }

                    updateProtocoleUI(p);
                    $('#protocoleContainer').slideDown();

                    // Auto-prescription
                    setTimeout(() => { $('#btnApplyProtocole').trigger('click'); }, 300);
                } else {
                    $('#protocoleContainer').hide();
                    $('#btnApplyProtocole').data('protocole', null);
                }
            },
            error: function() { $('#protocoleContainer').hide(); }
        });
    });
    ...
    function updateProtocoleUI(p) {
        ...
        if (p.traitement_principal) {
            ...
            let inStock = 0;
            meds.forEach(mName => {
                if (medicamentsList.find(m => m.nom.toLowerCase().includes(mName.trim().toLowerCase()) && m.stock_actuel > 0)) inStock++;
            });
            let badge = $('#stockBadge');
            if (inStock === meds.length) badge.removeClass('bg-danger bg-warning').addClass('bg-success').text('En stock ✅');
            else if (inStock > 0) badge.removeClass('bg-success bg-danger').addClass('bg-warning text-dark').text('Stock partiel ⚠️');
    ...
    $('#btnApplyProtocole').click(function() {
        ...
        if (p.traitement_principal) {
            ...
            medsToSearch.forEach(medName => {
                ...
                let matchedMed = medicamentsList.find(m => m.nom.toLowerCase().includes(searchTerm) || searchTerm.includes(m.nom.toLowerCase()));

                // Anti-doublon
                let alreadyExists = false;
                if (matchedMed) {
                    $('.selectMedicament').each(function() { if ($(this).val() == matchedMed.id) alreadyExists = true; });
                }
                ...
                if (!alreadyExists) {
                    ...
                    let label = matchedMed ? '-- Sélectionner --' : '-- Choisir l\'équivalent (' + medName.trim() + ') --';
                    let opts = '<option value="">' + label + '</option>';
                    medicamentsList.forEach(m => {
                        let sel = (matchedMed && m.id == matchedMed.id) ? 'selected' : '';
                        opts += `<option value="${m.id}" data-prix="${m.prix_vente}" data-stock="${m.stock_actuel}" ${sel}>${m.nom}</option>`;
                    });

                    let rowClass = matchedMed ? 'table-success' : 'table-warning';
                    let alertHtml = !matchedMed ? '<small class="text-danger fw-bold"><i class="fas fa-exclamation-triangle"></i> Non trouvé en stock</small>' : '';
        ...
        Toast.fire({ icon: 'success', title: 'Traitement généré !' });
        setTimeout(() => { $('.auto-added-row').removeClass('table-success'); }, 3000);
    });
    ...
    function updateMedicamentOptions() {
        ...
        $('.selectMedicament').each(function() {
            ...
            $(this).find('option').each(function() {
                $(this).toggle($(this).val() === "" || !selected.includes($(this).val()) || $(this).val() === cur);
            });
        });
    }
    ...
    $('#btnAjouterMedicament').click(function() {
        ...
        let row = `<tr>
            <td><select name="medicaments[]" class="form-select js-select2 form-select-sm selectMedicament" required>
                <option value="">-- Sélectionner --</option>
                @foreach ($medicaments as $med)
                <option value="{{ $med->id }}" data-prix="{{ $med->prix_vente }}" data-stock="{{ $med->stock_actuel }}">{{ $med->nom }}</option>
                @endforeach
            </select></td>
        ...
        let newRow = $(row);
        $('#ordonnanceTable tbody').append(newRow);
        newRow.find('.js-select2').select2({ width: '100%', placeholder: "-- Sélectionner --", allowClear: true });
        updateMedicamentOptions();
    });
    ...
    function calculateTotals() {
        $('#ordonnanceTable tbody tr').each(function() {
            ...
            if (qty > stock) row.find('.input-qty').addClass('is-invalid').attr('title', 'Stock insuffisant ! (Max: ' + stock + ')');
    ...
    $(document).on('click', '.btnSupprimer', function() {
        ...
        updateMedicamentOptions(); calculateTotals();
    });
    ...
    $(document).on('click', '.btnSupprimerAnalyse', function() {
        ...
        if ($('#analyseList li').length === 0) $('#analyseList').html('<li class="list-group-item text-center text-muted py-3" id="emptyAnalyseRow"><i class="fas fa-flask me-2"></i>Aucune analyse</li>');
    });
    ...
    $(document).on('click', '.btnSupprimerRdv', function() {
        $(this).closest('li').remove();
        if ($('#rdvList li').length === 0) $('#rdvList').html('<li class="list-group-item text-center text-muted py-3" id="emptyRdvRow"><i class="fas fa-calendar-check me-2"></i>Aucun rendez-vous</li>');
    });
    ...
    $(document).on('click', '.select-atlas-maladie', function() {
        ...
        $('html, body').animate({ scrollTop: $("#ordonnanceTable").offset().top - 100 }, 800);
    });
    ...
    $(document).on('change', '.compare-check', function() {
        let id = $(this).data('id'), nom = $(this).data('nom');
        if ($(this).is(':checked')) {
            if (selectedForCompare.length >= 2) { $(this).prop('checked', false); Toast.fire({ icon: 'warning', title: 'Max 2 protocoles' }); return; }
            selectedForCompare.push({ id, nom });
        } else { selectedForCompare = selectedForCompare.filter(i => i.id != id); }
        updateCompareBar();
    });

    function updateCompareBar() {
        $('#compareCount').text(selectedForCompare.length);
        selectedForCompare.length > 0 ? $('#compareBar').removeClass('d-none') : $('#compareBar').addClass('d-none');
    ...
    function fillCompareColumn(idx, p) {
        ...
        let meds = (p.traitement_principal || '').split(/[\+\,]/);
        let inStock = medicamentsList.filter(m => meds.some(s => m.nom.toLowerCase().includes(s.trim().toLowerCase()) && m.stock_actuel > 0)).length;
        let badgeClass = inStock === meds.length ? 'text-success' : (inStock > 0 ? 'text-warning' : 'text-danger');
        let badgeText = inStock === meds.length ? 'En stock' : (inStock > 0 ? 'Stock partiel' : 'Rupture');
        $(`#compStock${idx}`).html(`<span class="badge bg-light ${badgeClass} border"><i class="fas fa-box me-1"></i> ${badgeText}</span>`);

        $(`#btnApplyComp${idx}`).off('click').on('click', function() {
            $('#maladieSelect').val(p.maladie_id).trigger('change');
            setTimeout(() => { if ($('#btnApplyProtocole').is(':visible')) $('#btnApplyProtocole').trigger('click'); }, 800);
    ...
    $('#consultationForm').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({ icon: 'success', title: 'Succès', text: response.data || 'Consultation enregistrée !' }).then(() => {
                    if (!form.find('input[name="_method"]').length) {
                        form.trigger('reset');
                        $('#ordonnanceTable tbody').html('<tr id="emptyOrdonnanceRow"><td colspan="7" class="text-center text-muted py-3"><i class="fas fa-pills me-2"></i>Aucun médicament ajouté</td></tr>');
                        $('#analyseList').html('<li class="list-group-item text-center text-muted py-3" id="emptyAnalyseRow"><i class="fas fa-flask me-2"></i>Aucune analyse</li>');
                        $('#rdvList').html('<li class="list-group-item text-center text-muted py-3" id="emptyRdvRow"><i class="fas fa-calendar-check me-2"></i>Aucun rendez-vous</li>');
                        updatePatientTicket(); updateIMC();
                    }
                    if (response.redirect) window.location.href = response.redirect;
                });
            },
            error: function(xhr) {
                let errors = xhr.responseJSON?.errors, msg = '';
                if (errors) $.each(errors, function(k, v) { msg += v[0] + '\n'; });
